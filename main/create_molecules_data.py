...
def create_molecules_data(frames, nbr_molecules=20, size_image=2500, randomize=True, value=100000, ii_sd=0, off_length_min=1, off_length_max=3, number_blink_min=1, number_blink_max=3, own_blink=None, edge=0, grid=False, circle=False, num_circle=0):
    ...
    for j in range(nbr_molecules, nbr_molecules+20):
        if (grid == True) or (circle == True):
            data[j] = {'coordinates': generate_coordinates(size_image, edge), 'intensity': generate_intensity(value/2, value/10), 'on_times': generate_on_times(frames, randomize=randomize, off_length_min=frames-1, off_length_max=frames-1, number_blink_min=0, number_blink_max=1, beads=True)}
    return data
